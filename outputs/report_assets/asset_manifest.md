...
- **F10** `outputs/report_assets/main_figures/q3_paper_scenario_min_cost_heatmap.png`：Q3 24 场景最小吨氨成本方案热力图。正文主图，用于回答问题三第一小问；格内展示最小吨氨成本、对应日产量和绿电直连指标满足类型。 状态：OK
- **F12** `outputs/report_assets/main_figures/q3_vs_q2_multi_metric_comparison.png`：Q3 相对 Q2 的多指标变化图。正文主图，用于回答问题三第三小问；比较吨氨成本、购电量、上网电量、自发自用率、绿电比例和新能源上网比例。 状态：OK
...
- **T17** `outputs/report_assets/main_tables/q4_paper_24_scenario_summary.csv`：Q4 24 场景离网储能逐场景结果表。包含 scene、production、ton_cost、curtailment、wind_util、pv_util。 状态：OK
- **T18** `outputs/report_assets/main_tables/q4_paper_grid_vs_offgrid_comparison.csv`：Q4 联网与离网年化经济性对比表。包含 mode、annual_cost、annual_production、ton_cost。 状态：OK
