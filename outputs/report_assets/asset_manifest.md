...
- **T17** `outputs/report_assets/main_tables/q4_paper_24_scenario_summary.csv`：Q4 24 场景离网储能逐场景结果表。正文主表或核心支撑表，包含 scene、production、ton_cost、curtailment、wind_util、pv_util，用于回答 24 场景下储能参与后的运行结果。 状态：OK
- **T18** `outputs/report_assets/main_tables/q4_paper_grid_vs_offgrid_comparison.csv`：Q4 联网与离网年化经济性对比表。正文主表，包含 mode、annual_cost、annual_production、ton_cost，用于支撑联网同产量方案与离网储能方案的经济性比较。 状态：OK
...
- **AT16** `outputs/report_assets/appendix_tables/q4_offgrid_storage_scenario_summary.csv`：Q4 离网有储能逐场景原始汇总表。附录表，保留 Q4 离网储能模型对 24 场景的原始汇总输出。 状态：OK
- **AT17** `outputs/report_assets/appendix_tables/q4_offgrid_storage_hourly_dispatch.csv`：Q4 离网有储能逐小时调度表。附录表，保留储能充放电、SOC、弃电和制氨负荷等逐小时调度细节。 状态：OK
- **AT18** `outputs/report_assets/appendix_tables/q4_offgrid_storage_annual_summary.csv`：Q4 离网有储能年化汇总表。附录表，保留 Q4 离网储能方案的年化统计结果。 状态：OK
- **AT19** `outputs/report_assets/appendix_tables/q4_grid_same_production_scenario_summary.csv`：Q4 联网同产量逐场景汇总表。附录表，保留联网同产量对照方案的逐场景结果。 状态：OK
- **AT20** `outputs/report_assets/appendix_tables/q4_grid_same_production_hourly_dispatch.csv`：Q4 联网同产量逐小时调度表。附录表，保留联网对照方案的逐小时购电、售电和制氨负荷细节。 状态：OK
- **AT21** `outputs/report_assets/appendix_tables/q4_grid_vs_offgrid_annual_comparison.csv`：Q4 联网与离网年化原始对比表。附录表，保留联网与离网方案年化对比的完整原始字段。 状态：OK
