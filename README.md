...
| 容量 | 名称 | 含义 | 论文地位 |
|---:|---|---|---|
| 20 MWh | 低投入经济入口容量 | 单位成本收益较高，适合资金受限或低配储试点 | 辅助结论 |
| 110–115 MWh | 技术经济拐点共识区间 | 多方法识别出的边际收益递减转折区间 | 核心分析结果 |
| 115 MWh | 深度消纳型工程推荐容量 | 综合制氨增产、弃电削减和容量冗余后的推荐方案 | Q4 主结论 |
| 150 MWh | 技术饱和容量 | 接近最大技术收益，但继续扩容的边际收益较低 | 上界参考，不作为经济推荐 |

因此，本文不将 20 MWh 与 115 MWh 视为同一类推荐结果。20 MWh 是低投入经济入口容量，反映单位投资收益较高；115 MWh 是深度消纳型工程推荐容量，反映系统在风光消纳、制氨增产和储能冗余之间的技术经济平衡。

本文采用如下步骤识别深度消纳型工程推荐容量：

1. 以 5 MWh 为步长，对储能容量进行细步长扫描；
2. 对每个容量计算全年吨氨成本、年制氨量、弃电量和产能利用率；
3. 分别从年制氨量提升、弃电削减量和产能利用率提升三个维度识别容量拐点；
4. 采用几何弦距法、标准化收益—成本平衡法和分段线性 BIC 法进行交叉验证；
5. 将多方法、多指标共同支持的容量区间作为拐点共识区间，并取区间上界作为偏保守的工程推荐容量。

细步长扫描结果表明：

| 容量层级 | 容量 | 含义 |
|---|---:|---|
| 经济入口容量 | 20 MWh | 低投入下单位成本收益较高，但深度消纳能力有限 |
| 拐点区间下界 | 110 MWh | 储能收益由快速增长段进入拐点区间 |
| 工程推荐容量 | 115 MWh | 年制氨量提升与弃电削减共同支持的深度消纳型推荐容量 |
| 技术饱和容量 | 150 MWh | 年制氨量达到最大提升 99% 的最小容量 |

Q4 的 MILP 目标函数采用大惩罚系数加权目标近似表示调度优先级：优先避免缺供，在缺供受控的前提下提高制氨产量，并减少弃电。由于该目标函数存在权重设定，本文进一步进行 Q4 目标权重敏感性分析。结果显示，在缺供惩罚、产量奖励和弃电惩罚分别扰动后，推荐区间均保持为 110–115 MWh，工程推荐容量均保持为 115 MWh。这说明 115 MWh 的推荐结果不是由单一目标权重调参造成，而主要由源荷错配结构和储能边际收益递减规律共同决定。

因此，本文最终采用 **115 MWh** 作为 Q4 深度消纳型工程推荐储能容量；20 MWh 仅作为低投入经济入口容量，用于说明小容量储能在单位收益意义下具有较高经济性。
